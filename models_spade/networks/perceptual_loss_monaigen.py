...
from preloaded_models.resnet import ResNet, BasicBlock
...
class PerceptualLoss(nn.Module):
    ...
    def __init__(
        self, spatial_dims: int, network_type: str = "alex", is_fake_3d: bool = True, fake_3d_ratio: float = 0.5,
            from_preloaded = False, path_net = None, device = None,
    ):
        super().__init__()
        if device is None:
...
class MedicalNetPerceptualSimilarity(nn.Module):
    ...
    def __init__(self, net: str = "medicalnet_resnet10_23datasets", verbose: bool = False,
                 from_preloaded=False, path_net = None, device = None) -> None:
        super().__init__()
        if device is None:
        ...
        if from_preloaded:
            ...
            pretrained_state_dict = torch.load(path_net)
            ...
            self.model = torch.hub.load("Warvito/MedicalNet-models", model=net, verbose=verbose, force_reload=False)
